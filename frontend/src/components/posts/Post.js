    ...
    return <Card className={classes.container} >
        <div className={classes["author"]}>
            <Link to={`/profile/${props.authorId}`}>
                {!props.avatar && <Avatar className={classes["post-avatar"]} online={onlineStatus} src={require("../../images/"+`${defaultImagePath}`)} alt="" width={"50px"}/>}
                {props.avatar && <Avatar className={classes["post-avatar"]} online={onlineStatus} src={props.avatar} alt="" width={"50px"}/>}
            </Link>
            <Link to={`/profile/${props.authorId}`}>
